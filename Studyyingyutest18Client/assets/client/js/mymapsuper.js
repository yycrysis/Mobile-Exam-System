...
function initMap(){
    ...
    map.setCurrentCity("重庆");          // 设置地图显示的分类 此项是必须设置的
...
//    map.addOverlay(marker2);              // 将标注添加到地图中
    ...
    ajaxCallback("listGood",{type:2},function(list){
        parklist = list;
        var points = [];
        // 创建宠物编号解析器实例
...
function setSearchDiv(){
    ...
    cr.addCopyright({id: 1, content: "<div style='position: relative;right: 150px;top:8px;'><input placeholder='输入宠物编号搜索...' id='saddress' /><button onclick='getPointByAddress();'>搜索</button></div>", bounds: bs});
...
function addPolyline(points){
    var polyline = new BMap.Polyline(points, {strokeColor:"blue", strokeWeight:6, strokeOpacity:0.5});  //定义折线
    map.addOverlay(polyline);     //添加折线到地图上
}