    ...
    $('#grid1').datagrid({
        ...
        columns: [
            [
                ...
                //{title: '学习日志编号', width: 50, field: 'id', sortable: true},
                {title: '内容', width: 300, field: 'title', sortable: true},
                //{title: '学习日志方式', width: 100, field: 'way', sortable: true},
                //{title: '总价', width: 100, field: 'total', sortable: true},
                {title: '学生', width: 50, field: 'username',sortable: true},
           ...
           /*  {
            ...
            '-',
            {
                ...
                handler: function () {
                    ...
                    if (selected) {
                        $.post("<%=__APP__%>/Rizhi!jiaohao",{bid:selected.id,statecn:"通过"},function(){
                        	closeFlush();
                        });
                    } else {
            ...
            '-',{
                ...
                handler: function () {
                    ...
                    var selected = $('#grid1').datagrid('getSelected');
                    if (selected) {
                        $.post("<%=__APP__%>/Rizhi!jiaohao",{bid:selected.id,statecn:"未通过"},function(){
                        	closeFlush();
                        });
                    } else {
                        $.messager.alert("提示", "请选择一条记录进行操作");
...
function deleteItem(uuid) {
    openBackGround();
    $.post("<%=__APP__%>/Rizhi!deleteItem", {id: uuid}, function (data) {
        closeBackGround();
        closeFlush();
    });
}
